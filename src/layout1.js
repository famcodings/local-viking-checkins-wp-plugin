...
function getCheckInMapCardHTML(checkIn) {
  return `
    <div class="card col-6 map-check-in" id="map-check-in-${checkIn.id}">
      <div class="row">
        <div class="col-3 p-0">
          <div class="map"></div>
        </div>
...
function getAllCheckInMapCardsHTML(checkIns) {
  return checkIns.map(checkIn => getCheckInMapCardHTML(checkIn)).join("");
}

function initMap() {
  const elems = document.querySelectorAll(".map-check-in");
  checkIns.forEach((checkIn, i) => {
    if (!checkIn.coords || !Object.keys(checkIn.coords).length) {
      return
    }
    const elem = document.getElementById(`map-check-in-${checkIn.id}`);
    const mapElem = elem.querySelector(".map");
...
document.getElementById("map-checkIns").innerHTML = getAllCheckInMapCardsHTML(checkIns);
